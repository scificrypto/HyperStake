...
bool static Socks4(const CService &addrDest, N_SOCKET& hSocket)
{
...
bool static Socks5(string strDest, int port, N_SOCKET& hSocket)
{
    ...
    switch (pchRet2[3])
    {
        ...
        default: close_socket(hSocket); return error("Error: malformed proxy response");
    }
    if (ret)
    {
        close_socket(hSocket);
...
bool static ConnectSocketDirectly(const CService &addrConnect, N_SOCKET& hSocketRet, int nTimeout)
{
    ...
    N_SOCKET hSocket = socket(((struct sockaddr*)&sockaddr)->sa_family, SOCK_STREAM, IPPROTO_TCP);
...
bool ConnectSocket(const CService &addrDest, N_SOCKET& hSocketRet, int nTimeout)
{
    proxyType proxy;
    ...
    N_SOCKET hSocket = INVALID_SOCKET;
...
bool ConnectSocketByName(CService &addr, N_SOCKET& hSocketRet, const char *pszDest, int portDefault, int nTimeout)
{
    string strDest;
    int port = portDefault;
