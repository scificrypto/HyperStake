...
*Setup instructions for a gitian build of Element using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Element
...
Clone the git repositories for Element and gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/element/element
```
...
Follow the instructions in [doc/release-process.md](release-process.md) in the element repository
...
To build Element (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the element repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/element/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/element/element
...
For example:
```bash
URL=https://github.com/robvanmieghem/element.git
COMMIT=master
./bin/gbuild --commit element=${COMMIT} --url element=${URL} ../element/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit element=${COMMIT} --url element=${URL} ../element/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit element=${COMMIT} --url element=${URL} ../element/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/element-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/element-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/element-osx-build.assert
```
...
[element/gitian.sigs](https://github.com/element/gitian.sigs/) repository, or if that's not possible create a pull
